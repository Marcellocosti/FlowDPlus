...
trial_fit() {
    ...
    nice -n 10 python3 ~/alice/DmesonAnalysis/run3/flow/BDT/run_cutvar.py $1 \
                                                                          $config_flow \
                                                                          $usepreprocessed \
                                                                          $calc_weights \
                                                                          $make_yaml \
...
prompt_v2() {
    ...
    cmd=(nice -n 10 python3 "/home/mdicosta/alice/DmesonAnalysis/run3/flow/BDT/ComputeV2vsFDFrac.py" "$config_file" \
...
export config_modifies="/home/mdicosta/FlowDplus/FinalResults/4050final/multitrial_ptdiff_morebins/fit_config_modifications.yml"
export config_default="/home/mdicosta/FlowDplus/FinalResults/4050final/config_4050_combined.yml" # bdt cut of corr and uncorr, skip_cut, sysematic
export combPath="/home/mdicosta/FlowDplus/FinalResults/4050final/cutvar_combined"

export n_parallel=60

export output_dir="/home/mdicosta/FlowDplus/FinalResults/4050final/multitrial_ptdiff_morebins"
export syst_paths=$output_dir
export multitrial_config_path="/home/mdicosta/FlowDplus/FinalResults/4050final/multitrial_ptdiff_morebins"
...
nice -n 10 python3 /home/mdicosta/alice/DmesonAnalysis/run3/flow/systematics/make_yaml_for_syst_ptdiff.py $config_default -m $config_modifies -o $output_dir -mb
yaml_files=($(find "$multitrial_config_path" -type f -name "config_var_*.yml" -print0 | xargs -0 -n1))
echo "Yaml files generated!"
echo "${yaml_files[@]}"

# Store all pt_* directories in an array
...
for dir in "${pt_dirs[@]}"; do
    ...
    echo "${yaml_files[@]}"
    echo "${#yaml_files[@]}"
    printf "%s\n" "${yaml_files[@]}" | nice -n 10 parallel -j $n_parallel --progress --eta trial_fit {}
    echo "Multitrial fits performed!"

    # repeat the V2VsFrac estimation
    find "$dir" -type d -name "V2VsFrac" -exec rm -rf {} +
    nice -n 10 parallel -j $n_parallel --progress --eta prompt_v2 {1} ::: "${yaml_files[@]}"
    find "$dir" -type d -name "syst_summary" -exec rm -rf {} +
    nice -n 10 python3 ~/alice/DmesonAnalysis/run3/flow/systematics/compute_syst_multitrial_bdt.py "$dir" "/home/mdicosta/FlowDplus/FinalResults/4050final/cutvar_combined/" -o $dir --prompt
    
done

nice -n 10 python3 "/home/mdicosta/alice/DmesonAnalysis/run3/flow/systematics/merge_syst_pt_bins.py" $output_dir
